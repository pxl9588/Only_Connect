...
const Rectangle = forwardRef(({ ...props }, ref) => {
    ...
    //VOWELS
        ...
        text_size = "text-2xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-5xl";
    }
    ...
    return (
        <div
            ...
            className={`${width} ${height} ${color} shadow-2xl rounded-xl ${
                props.hidden ? "invisible" : ""
            }`}
        >
            <h1
                className={`${text_size} ${font_color} w-full h-full flex justify-center items-center text-center select-none`}
            >
                {props.children}
            </h1>
        </div>
